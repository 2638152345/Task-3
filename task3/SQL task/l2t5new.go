...
//t2:
func GetLowStockProducts(db *gorm.DB,threshold int)([]Product, error){
  var products []Product
  err := db.
...
//			SUM(order_items.quantity * order_items.unit_price) AS amount`).
		// Joins 关联订单项表，用于统计商品数量和计算销售总额
//		Joins("JOIN products ON projects.id = order_items.project_id").
//    Joins("JOIN orders ON order.id = order_items.order_id").
		// Where 只统计已支付的订单
//		Where("orders.status = ?", "PAID").
		// Group 按日期分组，将同一天的订单聚合在一起
//		Group("products.id,product.name").
		// Scan 将聚合结果映射到 SalesSummary 结构体
		// 注意：聚合查询必须使用 Scan 而不是 Find
//		Scan(&rows).Error
//	return rows, err
...
func CancelOrder(db *gorm.DB, orderID uint) error {
  return db.Transaction(func(tx*gorm.DB) error {
    ...
    if order.Status != "PANDING" {
      return errors.new("order can not be cancelled")
    }

    for _, item := range Items {
      if err := tx.Model(&Product{}).
      Where("id = ?",item.ProductID).
      UpdateColumn("stock", grom.Expr("stock + ?",item.Quantity)).
      Error; err != nil{
        return err
        }
    }
